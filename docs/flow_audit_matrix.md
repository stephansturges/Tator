...
| DS-002 | Dataset delete + restore | `DELETE /datasets/{dataset_id}`, `DELETE /qwen/datasets/{dataset_id}`, `/datasets/trash*` | `api/datasets.py -> delete_dataset_entry`, `delete_qwen_dataset`, `restore_dataset_trash_entry` | cpu_clean | not_required | fixed | Managed registry/Qwen deletes now soft-delete to backend trash with restore; linked deletes still remove only the registry record/overlays. |
