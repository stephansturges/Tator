...
def test_class_analysis_rolls_back_when_thread_start_fails(monkeypatch):
    ...
    with api.CLASS_ANALYSIS_RUN_FINGERPRINTS_LOCK:
        api.CLASS_ANALYSIS_RUN_FINGERPRINTS.clear()

    monkeypatch.setattr(api, "_class_analysis_run_fingerprint", lambda _payload: "test-fingerprint")
    monkeypatch.setattr(api.threading, "Thread", _FailStartThread)
    ...
    assert api.CLASS_ANALYSIS_RUN_FINGERPRINTS == {}
...
def test_clip_training_late_cancel_skips_artifact_publish(monkeypatch, tmp_path):
    with api.TRAINING_JOBS_LOCK:
        api.TRAINING_JOBS.clear()

    temp_dir = tmp_path / "clip_train_staging"
    temp_dir.mkdir()
    job = api.ClipTrainingJob(job_id="clip_cancel_publish", temp_dir=str(temp_dir))
    publish_calls = {"count": 0}

    def fake_train_clip_from_yolo(**_kwargs):
        job.cancel_event.set()
        return object()

    def fail_publish(_artifacts):
        publish_calls["count"] += 1
        raise AssertionError("cancelled CLIP job should not publish artifacts")
    ...
    monkeypatch.setattr(api, "train_clip_from_yolo", fake_train_clip_from_yolo)
    monkeypatch.setattr(api, "_publish_clip_training_artifacts", fail_publish)
...
def test_yolo_train_rejects_cuda_devices_on_macos_before_queue(monkeypatch, tmp_path):
    with api.YOLO_TRAINING_JOBS_LOCK:
        api.YOLO_TRAINING_JOBS.clear()
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("run dir should not be created")
        ),
    )
    monkeypatch.setattr(api, "torch", _FakeTorch(cuda=False, mps=True))
    monkeypatch.setattr(api.threading, "Thread", _NoThread)
    ...
    with pytest.raises(api.HTTPException) as exc_info:
        api.create_yolo_training_job(
            api.YoloTrainRequest(dataset_id="dataset_1", devices=[0], accept_tos=True)
        )

    assert exc_info.value.status_code == 400
    assert exc_info.value.detail == "yolo_cuda_devices_unavailable"
    assert api.YOLO_TRAINING_JOBS == {}
