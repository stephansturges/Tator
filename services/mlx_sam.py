...
_MLX_SAM_MODEL_LOCK = threading.Lock()
_MLX_SAM_EXECUTION_LOCK = threading.RLock()
_MLX_SAM_MODELS: Dict[Tuple[str, str], Tuple[Any, Any]] = {}
...
def build_mlx_sam_predictor() -> MlxSamPredictorAdapter:
    ...
    if not config.available or not config.model_path or not config.package_dir:
        raise MlxSamUnavailable(config.reason or "mlx_sam_unavailable")
    cache_key = (str(config.model_path.resolve()), str(config.package_dir.resolve()))
    with _MLX_SAM_MODEL_LOCK:
        cached = _MLX_SAM_MODELS.get(cache_key)
        if cached is None:
            sam_mod, predictor_mod = _load_external_mlx_sam_modules(config.package_dir)
            cached = (sam_mod.load(config.model_path), predictor_mod.SamPredictor)
            _MLX_SAM_MODELS[cache_key] = cached
        sam_model, predictor_cls = cached
    # Predictor state owns the image embedding; only immutable model weights
    # are shared across slots.
    predictor = predictor_cls(sam_model)
    return MlxSamPredictorAdapter(predictor)


def _evaluate_mlx_values(*values: Any) -> None:
    import mlx.core as mx

    mx.eval(*values)


def _clear_shared_mlx_sam_models_for_tests() -> None:
    with _MLX_SAM_MODEL_LOCK:
        _MLX_SAM_MODELS.clear()
