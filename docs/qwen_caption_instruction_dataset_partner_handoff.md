...
- The single-image and batch caption archive read routes, plus the legacy
  single-image and batch text-label mirror read routes, reject with
...
and archive copies disagree. A failed backend consistency check blocks training
readiness with `instruction_artifacts_inconsistent`, and the UI also refuses the
...
- generated-QA or caption0 targets whose QA id does not match a saved record
