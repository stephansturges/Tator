...
def test_agent_catalog_is_inference_only_not_training():
    for entry in AGENT_MODEL_OPTIONS:
        ...
        assert entry["smoke_status"] in {
            ...
            "mlx_vlm_qwen35_vision_alias_supported",
...
def test_agent_catalog_marks_qwen36_matrix_winners_separately():
    ...
    assert aeon_mlx["vision_inference_supported"] is True
    assert aeon_mlx["smoke_status"] == "mlx_vlm_qwen35_vision_alias_supported"
    assert "qwen3_5_vision" in aeon_mlx["compatibility_note"]
    assert "normalizes" in aeon_mlx["compatibility_note"]
