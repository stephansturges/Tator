...
def build_datasets_router(
    *,
    ...
    save_transient_fn: Callable[
        [str, Optional[str], Optional[str], Optional[str], Optional[str], Optional[int]], Any
    ],
    ...
    annotation_export_fn: Optional[Callable[..., Any]] = None,
    transient_annotation_export_fn: Optional[Callable[..., Any]] = None,
    annotation_export_prepare_fn: Optional[Callable[[str, dict], Any]] = None,
    transient_annotation_export_prepare_fn: Optional[Callable[[str, dict], Any]] = None,
    annotation_export_status_fn: Optional[Callable[[str, str], Any]] = None,
    transient_annotation_export_status_fn: Optional[Callable[[str, str], Any]] = None,
    ...
    caption_artifact_sets_fn: Optional[Callable[[], Any]] = None,
    caption_artifact_summary_fn: Optional[Callable[..., Any]] = None,
    export_caption_artifact_set_fn: Optional[Callable[[str, dict], Any]] = None,
) -> APIRouter:
    router = APIRouter()
    ...
    @router.get("/caption_artifacts/sets")
    def list_caption_artifact_sets():
        if caption_artifact_sets_fn is None:
            raise HTTPException(status_code=404, detail="caption_artifact_store_unavailable")
        return caption_artifact_sets_fn()

    @router.get("/datasets/{dataset_id}/caption_artifacts/summary")
    def get_caption_artifact_summary(
        dataset_id: str,
        caption_set_id: str = Query(""),
        image_names: str = Query(""),
        max_images: int = Query(200),
    ):
        if caption_artifact_summary_fn is None:
            raise HTTPException(status_code=404, detail="caption_artifact_store_unavailable")
        selected = [
            item.strip()
        ...
        return caption_artifact_summary_fn(
            dataset_id,
    ...
    @router.post("/datasets/{dataset_id}/caption_artifacts/export")
    def export_caption_artifact_set(dataset_id: str, payload: dict = Body(...)):  # noqa: B008
        if export_caption_artifact_set_fn is None:
            raise HTTPException(status_code=404, detail="caption_artifact_store_unavailable")
        return export_caption_artifact_set_fn(dataset_id, payload or {})
    ...
    def get_caption_coverage(
        ...
        image_names: str = Query(""),
        instruction_qa_imposed_questions: str = Query(""),
    ):
        ...
        imposed_questions = []
        raw_imposed = str(instruction_qa_imposed_questions or "").strip()
        if raw_imposed:
            try:
                loaded = json.loads(raw_imposed)
            except Exception:
                loaded = None
            if isinstance(loaded, list):
                imposed_questions = [str(item or "").strip() for item in loaded if str(item or "").strip()]
            else:
                imposed_questions = [item.strip() for item in raw_imposed.splitlines() if item.strip()]
        return caption_coverage_fn(
            ...
            image_names=selected,
            instruction_qa_imposed_questions=imposed_questions,
        )
    ...
    def save_transient_dataset(session_id: str, payload: dict = Body(...)):  # noqa: B008
        return save_transient_fn(
            ...
            (payload or {}).get("context"),
            (payload or {}).get("notes"),
            (payload or {}).get("expected_revision"),
        )
    ...
    @router.post("/datasets/{dataset_id}/annotation/export/prepare")
    def prepare_annotation_export(dataset_id: str, payload: dict = Body(default={})):  # noqa: B008
        if annotation_export_prepare_fn is None:
            raise HTTPException(status_code=404, detail="annotation_export_unavailable")
        return annotation_export_prepare_fn(dataset_id, payload or {})

    @router.get("/datasets/{dataset_id}/annotation/export/status")
    def get_annotation_export_status(dataset_id: str, job_id: str = Query(...)):  # noqa: B008
        if annotation_export_status_fn is None:
            raise HTTPException(status_code=404, detail="annotation_export_unavailable")
        return annotation_export_status_fn(dataset_id, str(job_id or "").strip())

    @router.get("/datasets/{dataset_id}/annotation/export")
    def export_annotations(dataset_id: str, token: str = Query("")):
        if annotation_export_fn is None:
            raise HTTPException(status_code=404, detail="annotation_export_unavailable")
        if str(token or "").strip():
            return annotation_export_fn(dataset_id, str(token).strip())
        return annotation_export_fn(dataset_id)
    ...
    @router.post("/datasets/transient/{session_id}/annotation/export/prepare")
    def prepare_transient_annotation_export(
        session_id: str, payload: dict = Body(default={})  # noqa: B008
    ):
        if transient_annotation_export_prepare_fn is None:
            raise HTTPException(status_code=404, detail="annotation_export_unavailable")
        return transient_annotation_export_prepare_fn(session_id, payload or {})

    @router.get("/datasets/transient/{session_id}/annotation/export/status")
    def get_transient_annotation_export_status(
        session_id: str, job_id: str = Query(...)  # noqa: B008
    ):
        if transient_annotation_export_status_fn is None:
            raise HTTPException(status_code=404, detail="annotation_export_unavailable")
        return transient_annotation_export_status_fn(session_id, str(job_id or "").strip())

    @router.get("/datasets/transient/{session_id}/annotation/export")
    def export_transient_annotations(session_id: str, token: str = Query("")):
        if transient_annotation_export_fn is None:
            raise HTTPException(status_code=404, detail="annotation_export_unavailable")
        if str(token or "").strip():
            return transient_annotation_export_fn(session_id, str(token).strip())
        return transient_annotation_export_fn(session_id)
