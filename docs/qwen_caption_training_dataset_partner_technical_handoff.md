...
This document is the primary standalone technical handoff for the
caption-derived VLM training-dataset work. It is written for an external
technical reviewer who needs to understand what was built, why it was built
this way, how an operator uses it, which artifacts are produced, which
validation gates protect the output, which failure modes were hardened, and
what proof is still required before a generated corpus should be used for
fine-tuning.
...
helper into an auditable training-dataset construction path. The table below is
the shortest way to evaluate the design intent.

| What was added | Why it was needed |
| --- | --- |
| A separate **Create VLM training dataset** action | Training-data creation has stricter provenance, review, and export requirements than ordinary captioning |
| `caption0` plus configurable generated visual QA | A single broad caption per image is usually too thin for VLM fine-tuning |
| Optional deterministic metadata QA | Label-derived count, class, presence, and simple spatial facts can be useful training signal when they are explicit and trusted |
| Strict separation between source annotations and generated language | Generated text can be plausible and wrong, so it must not become trusted label truth |
| Flat trainer JSONL with image/question/answer rows | The trainer needs a simple model-visible input shape rather than a construction archive |
| Per-image archive JSONL | Reviewers need to trace each selected or rejected row back to source evidence, generated candidates, and validation decisions |
| Candidate-level review JSONL and metadata-only import | Human review needs to accept, reject, or request revision without editing labels, boxes, image paths, or generated text through the review file |
| Run-level instruction report | A training packet needs readiness state, quality metrics, row counts, rejection reasons, settings fingerprints, and consistency proofs |
| Self-contained training bundle | External review and trainer dry runs should not depend on a mutable local image root |
| Semantic bundle manifest validation | A ZIP checksum proves bytes are unchanged, but not that the artifact set is coherent training data |
| Browser, backend, bundle, review-import, and trainer-loader gates | Training artifacts can be copied, edited, mixed, or generated by scripts, so every boundary must fail closed |
| Set-and-forget backend jobs | Dataset-scale VLM generation must survive refreshes, long runtimes, model loops, recoverable worker errors, and unattended operation |
| Prompt/token-budget cleanup | Thinking-capable models need larger automatic budgets, but explicit user caps must remain hard caps and prompt size must stay bounded |
| Dense-box prompt reduction | Authoritative counts must remain complete while prompts avoid dumping every box in large scenes |
| Streaming loop inspection and recovery | Repeated punctuation or token loops should be detected as failures, not accepted as captions |
| Model-availability styling | Operators need to know before a long run whether a selected model is local or requires download |
| Archive busy guards | Exports, imports, edits, and dataset operations must not race a running caption or instruction job |

The result is a workflow that produces training candidates and proof artifacts
together. It is intentionally conservative: unsafe rows remain visible for
audit, but do not silently enter trainer JSONL.
...
The manifest should be read as the bundle reproducibility proof. It answers:
"are these the files, bytes, roles, and copied assets this ZIP claims to
contain?" The report and embedded artifact-consistency objects answer the
training-readiness question: "do the trainer rows, archive rows, review rows,
settings, selected counts, and review states agree?" A reviewer should inspect
both. A checksum-only ZIP is not sufficient evidence for training use, and a
ready-looking report is not enough if the copied image and label payloads do
...
- **Bundle semantic drift**: bundle validation now reopens the finished ZIP and
  recomputes file roles, copied image/label inventories, bundled trainer rows,
  row counts, checksums, and trainer/archive/review/report consistency before
  returning the artifact.
- **Review identity drift**: review import and artifact validation require
  dataset identity, stable QA ids, canonical image-path matching, current text,
  and selected training-answer parity before review metadata is applied.
- **Settings drift**: instruction settings and settings fingerprints are
  recorded across the payload, archive, report, and per-image archive metadata;
  validators reject mixed artifacts from different row-family or answer-format
  settings.
...
- trainer, archive, review, report, copied-image, and copied-label row counts;
- required artifact membership for trainer, archive, review, report, and label
  map files;
...
The backend also validates the semantic contents behind the manifest. Trainer
JSONL rows are parsed and checked as trainer rows, archive and review JSONL
rows must be object rows, the report must parse as an object, manifest row
counts must match actual JSONL line counts, image and label inventories must
match actual copied assets, and the artifact-consistency proof is recomputed
from the bundled files. This catches hand-edited or stale bundles where every
individual file exists but the set no longer describes one coherent export.
...
- bundle construction, copied images, copied labels, trainer/archive/review
  ...
  asset inventories, row-count parity, file-role parity, trainer-row
  validation, checksum checks, and recomputed artifact consistency;
...
./.venv-macos/bin/python -m py_compile localinferenceapi.py api/datasets.py tests/test_labeling_panel_layout_contract.py tests/test_qwen_caption_dataset_job.py
...
The focused regression set for semantic bundle hardening is:

```bash
./.venv-macos/bin/python -m pytest \
  tests/test_qwen_caption_dataset_job.py::test_caption_instruction_bundle_manifest_validates_semantic_inventory \
  tests/test_qwen_caption_dataset_job.py::test_caption_instruction_bundle_manifest_rejects_semantic_drift \
  tests/test_qwen_caption_dataset_job.py::test_caption_instruction_bundle_manifest_recomputes_artifact_consistency \
  tests/test_labeling_panel_layout_contract.py::test_qwen_caption_instruction_bundle_formats_backend_failures \
  -q
```
