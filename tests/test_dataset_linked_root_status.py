...
def test_dataset_listing_ignores_hidden_transient_state_root(tmp_path) -> None:
    registry_root = tmp_path / "registry"
    state_root = registry_root / ".transient_sessions"
    state_root.mkdir(parents=True)
    # Even corrupted state that resembles registry metadata must never appear
    # as a dataset card.
    (state_root / "dataset_meta.json").write_text(
        json.dumps({"id": "not-a-dataset", "label": "Transient state"}),
        encoding="utf-8",
    )
