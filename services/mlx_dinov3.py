...
class MlxDinoV3Worker:
    ...
    def encode_pixels(
        ...
        *,
        ...
        """Encode a dense RGB uint8 batch without per-image filesystem I/O.

        ``pixels`` must already be resized to the DINOv3 processor input size
        and use NHWC RGB layout.  A single safetensors input replaces the old
        PNG/JPEG-per-view transport while the worker remains responsible for
        ImageNet normalization and model execution.
        """

        pixel_array = np.asarray(pixels)
        if (
            pixel_array.ndim != 4
            or pixel_array.shape[0] <= 0
            or pixel_array.shape[-1] != 3
        ):
            raise ValueError("pixels must have shape [batch, height, width, 3]")
        ...
        try:
            ...
            raise MlxDinoV3Unavailable(
                "safetensors.numpy is required for MLX-DINOv3 tensor transport"
            ) from exc
        ...
        with self._lock:
            ...
            try:
                root = Path(
                    os.environ.get("MLX_DINOV3_OUTPUT_ROOT", tempfile.gettempdir())
                ).expanduser()
                root.mkdir(parents=True, exist_ok=True)
                temp_dir = tempfile.TemporaryDirectory(
                    prefix="mlx_dinov3_pixels_",
                    dir=str(root),
                )
                temp_root = Path(temp_dir.name)
                input_path = temp_root / "pixels.safetensors"
                save_file({"pixels": pixel_array}, str(input_path))
                if output_path is None:
                    output_path = temp_root / "tokens.safetensors"
                output_path = Path(output_path)
                output_path.parent.mkdir(parents=True, exist_ok=True)
                request_id = uuid.uuid4().hex
                request = {
                ...
                response = self._read_json_line(timeout_seconds=self.timeout_seconds)
                if response.get("id") != request_id:
                    raise MlxDinoV3WorkerError(
                        "mlx_dinov3_worker_response_id_mismatch"
                    )
                if not response.get("ok"):
                    raise MlxDinoV3WorkerError(
                        str(response.get("error") or "mlx_dinov3_worker_failed")
                    )
                arrays = load_file(str(output_path))
                return {
