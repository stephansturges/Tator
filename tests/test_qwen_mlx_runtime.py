...
MIRIL_VLM2_MLX_ID = "MirilAI/Miril-DroneVLM-2B-2-MLX-4bit"
...
def test_qwen_model_registry_exposes_mlx_quantized_models():
    payload = api.list_qwen_models()
    models = payload["models"]
    ids = {entry["id"] for entry in models}

    assert payload["review_default"] == MIRIL_VLM2_MLX_ID
    assert MIRIL_VLM2_MLX_ID in ids
    ...
    miril_entry = next(entry for entry in models if entry["id"] == MIRIL_VLM2_MLX_ID)
    assert miril_entry["type"] == "builtin_mlx"
    assert miril_entry["metadata"]["runtime_platform"] == "mlx_vlm"
    assert miril_entry["metadata"]["model_family"] == "gemma4"
    assert miril_entry["metadata"]["model_line"] == "Miril Drone VLM2"
    assert miril_entry["metadata"]["vision_inference_supported"] is True
    assert miril_entry["metadata"]["training_supported"] is False
    assert miril_entry["metadata"]["load_strict"] is False
    assert "aerial" in miril_entry["metadata"]["dataset_context"]


def test_miril_vlm2_public_id_resolves_to_complete_configured_local_artifact(
    tmp_path, monkeypatch
):
    artifact = tmp_path / "miril-vlm2"
    artifact.mkdir()
    (artifact / "config.json").write_text('{"model_type":"gemma4"}', encoding="utf-8")
    (artifact / "model.safetensors").write_bytes(b"weights")
    monkeypatch.setattr(api, "MIRIL_DRONE_VLM2_MLX_LOCAL_PATH", str(artifact))

    resolved = api._effective_qwen_model_id_for_platform(
        MIRIL_VLM2_MLX_ID,
        api.QWEN_PLATFORM_MLX,
    )
    availability = api._qwen_model_local_state(
        MIRIL_VLM2_MLX_ID,
        api.QWEN_PLATFORM_MLX,
    )

    assert resolved == str(artifact.resolve())
    assert availability["model_id"] == MIRIL_VLM2_MLX_ID
    assert availability["effective_model_id"] == str(artifact.resolve())
    assert availability["local"] is True
    assert availability["needs_download"] is False
    assert api._qwen_mlx_load_strict_for_model(resolved) is False


def test_miril_vlm2_does_not_resolve_to_incomplete_local_artifact(tmp_path, monkeypatch):
    artifact = tmp_path / "incomplete-miril-vlm2"
    artifact.mkdir()
    (artifact / "config.json").write_text('{"model_type":"gemma4"}', encoding="utf-8")
    monkeypatch.setattr(api, "MIRIL_DRONE_VLM2_MLX_LOCAL_PATH", str(artifact))
    ...
    assert api._qwen_mlx_load_strict_for_model("mlx-community/Qwen3-VL-4B-Instruct-4bit") is True


def test_miril_vlm2_loader_forces_non_strict_weight_load_and_restores_module(monkeypatch):
    calls = {}

    class FakeModule:
        def load_weights(self, weights, strict=True):
            calls["weight_strict"] = strict
            return self

    original = FakeModule.load_weights

    def fake_load(model_id, **kwargs):
        calls["model_id"] = model_id
        calls["load_kwargs"] = dict(kwargs)
        FakeModule().load_weights([("weight", object())])
        return "model", "processor"

    monkeypatch.setattr(api, "MLX_VLM_LOAD", fake_load)

    result = api._load_qwen_mlx_with_weight_policy(
        MIRIL_VLM2_MLX_ID,
        load_kwargs={"lazy": False, "strict": False},
        module_cls=FakeModule,
    )

    assert result == ("model", "processor")
    assert calls["model_id"] == MIRIL_VLM2_MLX_ID
    assert calls["load_kwargs"]["strict"] is False
    assert calls["weight_strict"] is False
    assert FakeModule.load_weights is original
...
def test_qwen_mlx_runtime_never_passes_default_registry_alias_to_loader(monkeypatch):
    calls = {}
    ...
    runtime = api._load_qwen_mlx_runtime("default")

    assert runtime.platform == api.QWEN_PLATFORM_MLX
    assert runtime.model_id != "default"
    assert calls["model_id"] == runtime.model_id
    assert "/" in calls["model_id"]
...
def test_qwen_model_id_resolution_maps_default_registry_alias_before_loading():
    assert (
        api._effective_qwen_model_id_for_platform(
            "default",
    ...
    assert "/" in api._effective_qwen_model_id_for_platform(
        "default",
        api.QWEN_PLATFORM_MLX,
    )
