...
def test_save_transient_dataset_requires_and_records_exact_source_revision(
    tmp_path, monkeypatch
) -> None:
    source_root = tmp_path / "linked_source"
    source_root.mkdir(parents=True, exist_ok=True)
    ...
    with api.DATASET_TRANSIENT_LOCK:
        api.DATASET_TRANSIENT_SESSIONS.clear()
        api.DATASET_TRANSIENT_SESSIONS["sess-revision"] = {
            "session_id": "sess-revision",
            "dataset_root": str(source_root),
            "label": "Transient DS",
            "classes": ["car"],
            "yolo_layout": "flat",
            "overlay_labels": {"train:img.jpg": ["0 0.5 0.5 0.2 0.2"]},
            "overlay_text": {},
            "_state_revision": 7,
        }
    ...
    assert exc_info.value.status_code == 409
    assert exc_info.value.detail == "transient_session_revision_conflict"
    assert not (registry_root / "stale-copy").exists()

    out = api.save_transient_dataset(
        "sess-revision",
        dataset_id="saved-revision",
        label="Saved Linked",
        context="",
        notes="",
        expected_revision=7,
    ...
    assert out["source_session_id"] == "sess-revision"
    assert out["source_session_revision"] == 7
    meta = json.loads(
        (registry_root / "saved-revision" / api.DATASET_META_NAME).read_text(encoding="utf-8")
    )
    assert meta["transient_source_session_id"] == "sess-revision"
    assert meta["transient_source_revision"] == 7


def test_save_transient_dataset_rolls_back_if_source_changes_during_copy(
    tmp_path, monkeypatch
) -> None:
    source_root = tmp_path / "linked_source"
    source_root.mkdir(parents=True, exist_ok=True)
    ...
    with api.DATASET_TRANSIENT_LOCK:
        api.DATASET_TRANSIENT_SESSIONS.clear()
        api.DATASET_TRANSIENT_SESSIONS["sess-race"] = {
    ...
    original_persist = api._persist_transient_overlays_to_entry

    def _persist_then_mutate(entry, session):
        original_persist(entry, session)

        def _concurrent_edit(current):
            current["annotation_notes"] = "concurrent edit"

        api._mutate_transient_session("sess-race", _concurrent_edit)

    monkeypatch.setattr(api, "_persist_transient_overlays_to_entry", _persist_then_mutate)

    with pytest.raises(api.HTTPException) as exc_info:
        api.save_transient_dataset(
            "sess-race",
            dataset_id="raced-copy",
            label="Raced Copy",
            context="",
            notes="",
    ...
    assert exc_info.value.status_code == 409
    assert exc_info.value.detail == "transient_session_changed_during_save"
    assert not (registry_root / "raced-copy").exists()
    with api.DATASET_TRANSIENT_LOCK:
        assert api.DATASET_TRANSIENT_SESSIONS["sess-race"]["_state_revision"] == 12
        assert api.DATASET_TRANSIENT_SESSIONS["sess-race"]["annotation_notes"] == "concurrent edit"
...
def test_annotation_export_contains_labels_captions_and_labelmap(tmp_path) -> None:
    dataset_root = tmp_path / "dataset"
    (dataset_root / "train" / "labels").mkdir(parents=True)
    (dataset_root / "train" / "images").mkdir(parents=True)
    (dataset_root / "train" / "text_labels").mkdir(parents=True)
    (dataset_root / "train" / "labels" / "frame.txt").write_text(
        "0 0.5 0.5 0.2 0.2\n",
        encoding="utf-8",
    )
    (dataset_root / "train" / "text_labels" / "frame.txt").write_text(
        "a caption",
        encoding="utf-8",
    )
    (dataset_root / "train" / "images" / "frame.jpg").write_bytes(b"not-exported")
    response = api._build_annotation_export_archive(
    ...
    try:
        with zipfile.ZipFile(response.path) as archive:
            assert archive.namelist() == [
                "labelmap.txt",
                "train/frame.txt",
                "text_labels/train/frame.txt",
            ]
            assert archive.read("labelmap.txt") == b"first\nsecond\n"
            assert archive.read("train/frame.txt") == b"0 0.5 0.5 0.2 0.2\n"
            assert archive.read("text_labels/train/frame.txt") == b"a caption"
            assert not any(name.endswith(".jpg") for name in archive.namelist())
    finally:
        shutil.rmtree(Path(response.path).parent, ignore_errors=True)


def test_annotation_export_prepare_is_revision_safe_and_reuses_cache(
    tmp_path, monkeypatch
) -> None:
    dataset_root = tmp_path / "dataset"
    ...
    record_root = tmp_path / "registry" / "dataset"
    ...
    monkeypatch.setattr(api, "DATASET_ANNOTATION_EXPORT_CACHE_ROOT", tmp_path / "cache")
    monkeypatch.setattr(api, "_resolve_dataset_entry", lambda _dataset_id: dict(entry))

    first = api.prepare_dataset_annotation_export(
        "dataset",
        {"expected_revision": 4},
    )
    second = api.prepare_dataset_annotation_export(
        "dataset",
        {"expected_revision": 4},
    )
    ...
    response = api.download_dataset_annotation_export("dataset", first["token"])
    with zipfile.ZipFile(response.path) as archive:
        assert archive.namelist() == ["labelmap.txt", "train/frame.txt"]
        assert archive.read("labelmap.txt") == b"object\n"

    metadata["annotation_revision"] = 5
    (record_root / api.DATASET_META_NAME).write_text(
        json.dumps(metadata),
        encoding="utf-8",
    )
    with pytest.raises(Exception) as exc_info:
        api.prepare_dataset_annotation_export(
            "dataset",
            {"expected_revision": 4},
        )
...
def test_annotation_export_overrides_saved_label_without_copying_source_image(tmp_path) -> None:
    dataset_root = tmp_path / "dataset"
    labels_root = dataset_root / "labels"
    labels_root.mkdir(parents=True)
    (dataset_root / "images").mkdir(parents=True)
    (labels_root / "frame.txt").write_text("0 old\n", encoding="utf-8")
    (dataset_root / "images" / "frame.jpg").write_bytes(b"not-exported")
    overlay_root = tmp_path / "overlay"
    overlay_root.mkdir()
    overlay_label = overlay_root / "frame.txt"
    overlay_label.write_text("1 new\n", encoding="utf-8")
    response = api._build_annotation_export_archive(
        dataset_root=dataset_root,
        yolo_layout="flat",
        file_overrides={"labels/frame.txt": overlay_label},
    )
    try:
        with zipfile.ZipFile(response.path) as archive:
            assert archive.namelist() == ["train/frame.txt"]
            assert archive.read("train/frame.txt") == b"1 new\n"
    finally:
        shutil.rmtree(Path(response.path).parent, ignore_errors=True)
...
def test_persistent_snapshot_text_only_update_preserves_existing_label_overlay(
    ...
    assert "annotation_record_revision" not in out["records"][0]
    assert out["records"][0]["annotation_source_identity"].startswith("asi1_")
...
def test_persistent_snapshot_updates_current_progress_without_manifest_scan(
    tmp_path, monkeypatch
) -> None:
    entry = _entry_for_annotation(tmp_path)
    ...
    (dataset_root / "text_labels").mkdir(parents=True)
    (dataset_root / "text_labels" / "img.txt").write_text(
        "existing caption",
        encoding="utf-8",
    )
    meta = {"annotation_lock": _active_lock("sess-lock")}
    api._annotation_set_progress_cache(
        meta,
        {
            "images_total": 6,
            "images_with_bbox_or_seg": 1,
            "images_with_text_label": 1,
        },
    )
    monkeypatch.setattr(api, "_resolve_dataset_entry", lambda _dataset_id: entry)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda _entry: pytest.fail("autosave must not rebuild the full manifest"),
    )
    ...
    assert out["progress"] == {
        "images_total": 6,
        "images_with_bbox_or_seg": 2,
        "images_with_text_label": 2,
        "percent_labeled": pytest.approx(100.0 / 3.0),
        "percent_text_labeled": pytest.approx(100.0 / 3.0),
    }
    assert api._annotation_progress_cache_value(meta) == out["progress"]


def test_persistent_snapshot_content_only_edit_needs_one_metadata_write(
    tmp_path, monkeypatch
) -> None:
    entry = _entry_for_annotation(tmp_path)
    dataset_root = Path(entry["dataset_root"])
    ...
    monkeypatch.setattr(
        api,
        "_annotation_persist_meta",
        lambda _entry, current_meta: persisted.append(json.loads(json.dumps(current_meta))),
    )
    ...
    assert api._annotation_progress_cache_value(persisted[0]) == out["progress"]
    assert out["progress"]["images_with_bbox_or_seg"] == 1


def test_persistent_snapshot_applies_duplicate_record_deltas_sequentially(
    tmp_path, monkeypatch
) -> None:
    entry = _entry_for_annotation(tmp_path)
    meta = {"annotation_lock": _active_lock("sess-lock")}
    ...
    assert out["progress"]["images_with_bbox_or_seg"] == 1
    assert out["progress"]["images_with_text_label"] == 0
    receipts = {
        row["image_relpath"]: row
        for row in out["records"]
    }
    assert receipts["img.jpg"]["annotation_record_revision"] == (
        api._annotation_image_label_revision([])
    )
    assert receipts["img_ok.jpg"]["annotation_record_revision"] == (
        api._annotation_image_label_revision(["0 0.5 0.5 0.1 0.1"])
    )
    overlay_root = Path(entry["registry_root"]) / api.DATASET_ANNOTATION_OVERLAY_DIRNAME
    assert (overlay_root / "labels" / "train" / "img.txt").read_text(encoding="utf-8") == ""
    assert (overlay_root / "text_labels" / "img.txt").read_text(encoding="utf-8") == ""
    assert (overlay_root / "labels" / "train" / "img_ok.txt").read_text(
        encoding="utf-8"
    ).strip()


def test_persistent_snapshot_keeps_unknown_progress_unknown_until_manifest_rebuild(
    tmp_path, monkeypatch
) -> None:
    entry = _entry_for_annotation(tmp_path)
    meta = {
        "annotation_lock": _active_lock("sess-lock"),
        # Legacy registration metadata can look complete while being wrong.
        "annotation_progress": {
            "images_total": 6,
            "images_with_bbox_or_seg": 0,
            "images_with_text_label": 0,
    ...
    collect_calls = 0
    collect_images = api._annotation_collect_images
    effective_label_lines = api._annotation_effective_label_lines
    effective_text_label = api._annotation_effective_text_label

    def counted_collect(entry_arg):
        nonlocal collect_calls
        collect_calls += 1
        return collect_images(entry_arg)
    ...
    monkeypatch.setattr(api, "_annotation_persist_meta", lambda _entry, _meta: None)
    monkeypatch.setattr(api, "_annotation_collect_images", counted_collect)
    monkeypatch.setattr(
        api,
        "_annotation_effective_label_lines",
        lambda *_args, **_kwargs: pytest.fail("unknown progress must not read old labels"),
    )
    monkeypatch.setattr(
        api,
        "_annotation_effective_text_label",
        lambda *_args, **_kwargs: pytest.fail("unknown progress must not read old text"),
    )
    ...
    assert out["progress"] == {}
    assert out["records"][0]["annotation_record_revision"] == (
        api._annotation_image_label_revision(["0 0.5 0.5 0.1 0.1"])
    )
    assert collect_calls == 0
    assert api._annotation_progress_cache_value(meta) is None

    monkeypatch.setattr(api, "_annotation_effective_label_lines", effective_label_lines)
    monkeypatch.setattr(api, "_annotation_effective_text_label", effective_text_label)
    manifest = api.get_dataset_annotation_manifest("ds")
    assert collect_calls == 1
    assert manifest["progress"]["images_with_bbox_or_seg"] == 1
    assert api._annotation_progress_cache_value(meta) == manifest["progress"]


def test_annotation_progress_cache_from_prior_backend_process_is_unknown() -> None:
    meta = {}
    api._annotation_set_progress_cache(
        meta,
        {
    ...
    meta["annotation_progress_cache_runtime_epoch"] = "previous-backend-process"

    assert api._annotation_progress_cache_value(meta) is None


def test_persistent_snapshot_write_failure_leaves_progress_cache_invalid(
    tmp_path, monkeypatch
) -> None:
    entry = _entry_for_annotation(tmp_path)
    meta = {"annotation_lock": _active_lock("sess-lock")}
    ...
    persisted = []
    write_calls = 0
    write_text = api._annotation_write_text_within_root

    def persist(_entry, current_meta):
        persisted.append(json.loads(json.dumps(current_meta)))

    def fail_second_write(path, root, text):
        nonlocal write_calls
        write_calls += 1
        if write_calls == 2:
            raise OSError("simulated disk failure")
        return write_text(path, root, text)

    monkeypatch.setattr(api, "_resolve_dataset_entry", lambda _dataset_id: entry)
    monkeypatch.setattr(
        api, "_annotation_load_or_create_meta", lambda _entry: (Path("/tmp/meta.json"), meta)
    )
    monkeypatch.setattr(api, "_annotation_persist_meta", persist)
    monkeypatch.setattr(api, "_annotation_write_text_within_root", fail_second_write)
    ...
    assert len(persisted) == 1
    assert persisted[0]["annotation_progress_cache_current"] is False
    assert api._annotation_progress_cache_value(meta) is None
    overlay_root = Path(entry["registry_root"]) / api.DATASET_ANNOTATION_OVERLAY_DIRNAME
    assert (overlay_root / "labels" / "train" / "img.txt").exists()
    assert not (overlay_root / "text_labels" / "img.txt").exists()


def test_persistent_snapshot_final_meta_failure_keeps_durable_cache_invalid(
    tmp_path, monkeypatch
) -> None:
    entry = _entry_for_annotation(tmp_path)
    meta = {"annotation_lock": _active_lock("sess-lock")}
    ...
    def fail_final_persist(_entry, current_meta):
        if persisted:
    ...
    monkeypatch.setattr(api, "_annotation_persist_meta", fail_final_persist)
    ...
    assert len(persisted) == 1
    durable_meta = persisted[0]
    assert api._annotation_progress_cache_value(durable_meta) is None
    overlay_root = Path(entry["registry_root"]) / api.DATASET_ANNOTATION_OVERLAY_DIRNAME
    assert (overlay_root / "labels" / "train" / "img.txt").exists()
    assert (overlay_root / "text_labels" / "img.txt").read_text(
        encoding="utf-8"
    ) == "saved overlay"


def test_set_text_label_invalidates_current_progress_before_presence_change(
    tmp_path, monkeypatch
) -> None:
    entry = _entry_for_annotation(tmp_path)
    ...
    out = api.set_text_label("ds", "img.jpg", {"caption": "new caption"})

    assert out == {"status": "saved", "caption": "new caption"}
    assert len(persisted) == 1
    assert api._annotation_progress_cache_value(persisted[0]) is None


def test_linked_session_start_and_heartbeat_do_not_scan_manifest(tmp_path, monkeypatch) -> None:
    entry = _entry_for_annotation(tmp_path)
    meta = {"annotation_lock": {}}
    monkeypatch.setattr(api, "_resolve_dataset_entry", lambda _dataset_id: entry)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        api,
        "_annotation_manifest_for_entry",
        lambda _entry: pytest.fail("session lock maintenance must not scan the manifest"),
    )

    started = api.start_dataset_annotation_session(
        "ds",
        {"session_id": "sess-lock", "editor_name": "test"},
    )
    heartbeat = api.heartbeat_dataset_annotation_session(
        "ds",
        {"session_id": "sess-lock", "editor_name": "test"},
    )
    ...
    assert heartbeat["lock"]["session_id"] == "sess-lock"
...
def test_annotation_overlay_text_write_is_private_and_fsynced(tmp_path, monkeypatch) -> None:
    overlay_root = tmp_path / "overlay"
    overlay_root.mkdir()
    label_path = overlay_root / "labels" / "train" / "img.txt"
    fsync_calls = []
    monkeypatch.setattr(api.os, "fsync", lambda fd: fsync_calls.append(fd))
    ...
    # One fsync covers the file contents; the second makes the atomic directory
    # entry replacement durable before the save can be acknowledged.
...
def test_transient_annotation_manifest_ignores_source_label_symlink_escape(
    ...
    monkeypatch.setattr(api, "DATASET_TRANSIENT_STATE_ROOT", tmp_path / "transient_state")
...
def test_transient_annotation_autosaves_survive_backend_restart(
    tmp_path, monkeypatch
) -> None:
    dataset_root = tmp_path / "dataset"
    (dataset_root / "images").mkdir(parents=True)
    ...
    (dataset_root / "labelmap.txt").write_text("car\n", encoding="utf-8")
    monkeypatch.setattr(api, "DATASET_LINK_ROOTS", [tmp_path.resolve()])
    state_root = tmp_path / "transient_state"
    monkeypatch.setattr(api, "DATASET_TRANSIENT_STATE_ROOT", state_root)

    opened = api.open_dataset_path(str(dataset_root), strict=True)
    session_id = opened["session_id"]
    lock_id = "browser-tab-lock"
    api.start_transient_annotation_session(
        session_id,
        {"session_id": lock_id, "editor_name": "test browser"},
    )
            ...
            "records": [
                {
                    ...
                    "text_label": "saved before restart",
                }
            ],
            "cursor": {"split": "train", "image_relpath": "img.jpg"},
        },
    )
    state_path = api._transient_session_state_path(session_id)
    assert state_path.is_file()
    assert state_path.stat().st_mode & 0o777 == 0o600

    # Simulate a new backend process: its in-memory registry starts empty, but
    # the browser still holds the same transient session id.
    with api.DATASET_TRANSIENT_LOCK:
        api.DATASET_TRANSIENT_SESSIONS.clear()

    restored = api.get_transient_annotation_manifest(session_id)

    restored_row = restored["images"][0]
    assert restored_row["label_lines"] == ["0 0.5 0.5 0.25 0.25"]
    assert restored_row["text_label"] == "saved before restart"
    assert restored["cursor"] == {"split": "train", "image_relpath": "img.jpg"}
    assert restored["lock"]["session_id"] == lock_id
    with api.DATASET_TRANSIENT_LOCK:
        assert session_id in api.DATASET_TRANSIENT_SESSIONS

    deleted = api.delete_transient_dataset(session_id)
    assert deleted["status"] == "deleted"
    assert not state_path.exists()


def test_transient_state_commit_fsyncs_file_and_parent_directory(
    tmp_path, monkeypatch
) -> None:
    state_root = tmp_path / "transient_state"
    monkeypatch.setattr(api, "DATASET_TRANSIENT_STATE_ROOT", state_root)
    ...
    def _record_fsync(fd: int) -> None:
        observed_modes.append(api.os.fstat(fd).st_mode)
        original_fsync(fd)

    monkeypatch.setattr(api.os, "fsync", _record_fsync)
    with api.DATASET_TRANSIENT_LOCK:
        stored = api._transient_session_state_write_locked(
            "fsync-session",
            {
                "session_id": "fsync-session",
    ...
    assert stored["_state_schema_version"] == api.DATASET_TRANSIENT_STATE_SCHEMA_VERSION
    assert any(stat.S_ISREG(mode) for mode in observed_modes)
    assert any(stat.S_ISDIR(mode) for mode in observed_modes)


def test_storage_health_probes_transient_session_state_root(
    tmp_path, monkeypatch
) -> None:
    transient_root = tmp_path / "transient_state"
    monkeypatch.setattr(api, "DATASET_TRANSIENT_STATE_ROOT", transient_root)
    observed = []
    monkeypatch.setattr(api, "_storage_probe_root", lambda root: observed.append(Path(root)))

    payload = api._storage_check_payload()

    roots_by_name = {entry["name"]: entry for entry in payload["roots"]}
    assert payload["ok"] is True
    assert roots_by_name["transient_dataset_state"]["path"] == str(transient_root)
    assert transient_root in observed


def test_transient_state_write_failure_does_not_publish_memory_mutation(
    tmp_path, monkeypatch
) -> None:
    dataset_root = tmp_path / "dataset"
    (dataset_root / "images").mkdir(parents=True)
    ...
    with api.DATASET_TRANSIENT_LOCK:
        before = api.copy.deepcopy(api.DATASET_TRANSIENT_SESSIONS[session_id])

    def _fail_write(_session_id, _session):
        raise api.HTTPException(status_code=500, detail="forced_persist_failure")

    monkeypatch.setattr(api, "_transient_session_state_write_locked", _fail_write)
    with pytest.raises(api.HTTPException) as exc_info:
        api.patch_transient_annotation_meta(
            session_id,
            {"session_id": lock_id, "notes": "must not be acknowledged"},
        )

    assert exc_info.value.detail == "forced_persist_failure"
    with api.DATASET_TRANSIENT_LOCK:
        after = api.DATASET_TRANSIENT_SESSIONS[session_id]
        assert after == before
...
def test_invalid_transient_state_is_quarantined_not_deleted(
    tmp_path, monkeypatch, payload
) -> None:
    monkeypatch.setattr(api, "DATASET_TRANSIENT_STATE_ROOT", tmp_path / "transient_state")
    session_id = "schema-mismatch" if payload.startswith(b"{") else "corrupt-json"
    state_path = api._transient_session_state_path(session_id)
    state_path.write_bytes(payload)

    with pytest.raises(api.HTTPException) as exc_info:
        api._resolve_transient_session(session_id)

    assert exc_info.value.status_code == 500
    assert exc_info.value.detail == "transient_session_state_invalid"
    assert not state_path.exists()
    quarantined = list((state_path.parent / ".invalid").iterdir())
    assert len(quarantined) == 1
    assert quarantined[0].read_bytes() == payload


def test_transient_state_symlink_is_quarantined_without_reading_target(
    tmp_path, monkeypatch
) -> None:
    state_root = tmp_path / "transient_state"
    monkeypatch.setattr(api, "DATASET_TRANSIENT_STATE_ROOT", state_root)
    session_id = "symlink-state"
    state_path = api._transient_session_state_path(session_id)
    ...
    quarantined = list((state_root / ".invalid").iterdir())
    assert len(quarantined) == 1
    assert quarantined[0].is_symlink()


def test_transient_state_root_symlink_fails_closed(tmp_path, monkeypatch) -> None:
    outside = tmp_path / "outside"
    outside.mkdir()
    marker = outside / "keep.txt"
    marker.write_text("keep", encoding="utf-8")
    linked_root = tmp_path / "transient_state"
    try:
        linked_root.symlink_to(outside, target_is_directory=True)
    except OSError as exc:
        pytest.skip(f"symlink unsupported: {exc}")
    monkeypatch.setattr(api, "DATASET_TRANSIENT_STATE_ROOT", linked_root)

    with pytest.raises(api.HTTPException) as exc_info:
        api._transient_session_state_path("blocked")

    assert exc_info.value.detail == "transient_session_state_root_invalid"
    assert marker.read_text(encoding="utf-8") == "keep"


def test_transient_state_purge_preserves_expired_overlay_for_lazy_resume(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setattr(api, "DATASET_TRANSIENT_STATE_ROOT", tmp_path / "transient_state")
    now = time.time()
    with api.DATASET_TRANSIENT_LOCK:
        active = api._transient_session_state_write_locked(
            "active-session",
            {
                "session_id": "active-session",
                "dataset_root": str(tmp_path / "active"),
                "expires_at": now + 60,
                "_state_revision": 0,
            },
        )
        expired = api._transient_session_state_write_locked(
            "expired-session",
            {
                "session_id": "expired-session",
                "dataset_root": str(tmp_path / "expired"),
                "expires_at": now - 1,
                "annotation_lock": _active_lock("stale-lock"),
                "overlay_text": {"train:img.jpg": "retained"},
                "_state_revision": 0,
            },
        )
        api.DATASET_TRANSIENT_SESSIONS.clear()

    removed = api._purge_expired_transient_sessions(now_ts=now, include_persisted=True)

    assert removed == 1
    assert api._transient_session_state_path(active["session_id"]).is_file()
    expired_path = api._transient_session_state_path(expired["session_id"])
    assert expired_path.is_file()

    resumed = api._resolve_transient_session(expired["session_id"])
    assert resumed["expires_at"] > now
    assert resumed["annotation_lock"] == {}
    assert resumed["overlay_text"] == {"train:img.jpg": "retained"}
    assert api.delete_transient_dataset(expired["session_id"])["status"] == "deleted"
    assert not expired_path.exists()


def test_transient_snapshot_and_heartbeat_commits_do_not_clobber_each_other(
    tmp_path, monkeypatch
) -> None:
    dataset_root = tmp_path / "dataset"
    (dataset_root / "images").mkdir(parents=True)
    ...
    monkeypatch.setattr(api, "DATASET_LINK_ROOTS", [tmp_path.resolve()])
    monkeypatch.setattr(api, "DATASET_TRANSIENT_STATE_ROOT", tmp_path / "transient_state")
    session_id = api.open_dataset_path(str(dataset_root), strict=True)["session_id"]
    lock_id = "browser-tab-lock"
    api.start_transient_annotation_session(
        session_id,
        {"session_id": lock_id, "editor_name": "test browser"},
    ...
    def _heartbeat():
        barrier.wait()
        return api.heartbeat_transient_annotation_session(
            session_id,
            {"session_id": lock_id, "editor_name": "test browser"},
        )
    ...
    def _snapshot():
        barrier.wait()
        return api.save_transient_annotation_snapshot(
            session_id,
            {
                "session_id": lock_id,
                "records": [
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        heartbeat_future = pool.submit(_heartbeat)
        snapshot_future = pool.submit(_snapshot)
        barrier.wait()
        assert heartbeat_future.result()["status"] == "ok"
        assert snapshot_future.result()["status"] == "saved"

    with api.DATASET_TRANSIENT_LOCK:
        api.DATASET_TRANSIENT_SESSIONS.clear()
    restored = api.get_transient_annotation_manifest(session_id)
    assert restored["lock"]["session_id"] == lock_id
    assert restored["images"][0]["label_lines"] == ["0 0.5 0.5 0.2 0.2"]


def test_transient_heartbeat_does_not_rewrite_large_overlay_but_snapshot_does(
    tmp_path, monkeypatch
) -> None:
    dataset_root = tmp_path / "dataset"
    (dataset_root / "images").mkdir(parents=True)
    ...
    def _seed_large_overlay(session):
        session["overlay_text"] = {
            f"train:synthetic-{idx}.jpg": "x" * 128 for idx in range(2000)
        }

    api._mutate_transient_session(session_id, _seed_large_overlay)
    original_write = api._transient_session_state_write_locked
    writes = []

    def _record_write(sid, session):
        writes.append((sid, int(session.get("_state_revision") or 0)))
        return original_write(sid, session)

    monkeypatch.setattr(api, "_transient_session_state_write_locked", _record_write)

    before_revision = api._resolve_transient_session(session_id)["_state_revision"]
    heartbeat = api.heartbeat_transient_annotation_session(
        session_id,
        {"session_id": lock_id, "editor_name": "test browser"},
    )
    assert heartbeat["status"] == "ok"
    assert heartbeat["session_revision"] == before_revision
    assert writes == []

    saved = api.save_transient_annotation_snapshot(
        session_id,
        {
            "session_id": lock_id,
            "records": [
    ...
    with api.DATASET_TRANSIENT_LOCK:
        api.DATASET_TRANSIENT_SESSIONS.clear()
    restored = api._resolve_transient_session(session_id)
    assert restored["annotation_lock"]["session_id"] == lock_id
    assert len(restored["overlay_text"]) == 2000
    assert restored["overlay_labels"]["train:img.jpg"] == ["0 0.5 0.5 0.2 0.2"]
    api.delete_transient_dataset(session_id)


def test_transient_snapshot_save_does_not_rebuild_full_manifest(
    tmp_path, monkeypatch
) -> None:
    dataset_root = tmp_path / "dataset"
    (dataset_root / "images").mkdir(parents=True)
    ...
    api.get_transient_annotation_manifest(session_id)
    monkeypatch.setattr(
        api,
        "get_transient_annotation_manifest",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("snapshot save must not scan the full manifest")
        ),
    )
    ...
    assert saved["status"] == "saved"
    assert saved["progress"]["images_total"] == 1
    assert saved["progress"]["images_with_bbox_or_seg"] == 1
    assert saved["records"][0]["annotation_record_revision"] == (
        api._annotation_image_label_revision(["0 0.5 0.5 0.2 0.2"])
    )
    assert saved["records"][0]["annotation_source_identity"].startswith("asi1_")
    api.delete_transient_dataset(session_id)


def test_transient_snapshot_and_meta_commits_do_not_clobber_each_other(
    tmp_path, monkeypatch
) -> None:
    dataset_root = tmp_path / "dataset"
    (dataset_root / "images").mkdir(parents=True)
    ...
    def _meta():
        barrier.wait()
        return api.patch_transient_annotation_meta(
            session_id,
            {
                "session_id": lock_id,
                "labelmap": ["car", "truck"],
                "notes": "concurrent meta",
            },
        )
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        meta_future = pool.submit(_meta)
        snapshot_future = pool.submit(_snapshot)
        barrier.wait()
        assert meta_future.result()["labelmap"] == ["car", "truck"]
        assert snapshot_future.result()["status"] == "saved"

    with api.DATASET_TRANSIENT_LOCK:
        api.DATASET_TRANSIENT_SESSIONS.clear()
    restored = api.get_transient_annotation_manifest(session_id)
    assert restored["labelmap"] == ["car", "truck"]
    assert restored["notes"] == "concurrent meta"
    assert restored["images"][0]["text_label"] == "concurrent caption"


def test_transient_delete_race_is_linearized_and_never_silently_persists(
    tmp_path, monkeypatch
) -> None:
    dataset_root = tmp_path / "dataset"
    (dataset_root / "images").mkdir(parents=True)
    ...
    def _mutate():
        ...
        try:
            return api.patch_transient_annotation_meta(
                session_id,
                {"session_id": lock_id, "notes": "racing mutation"},
            )
    ...
    def _delete():
        barrier.wait()
        return api.delete_transient_dataset(session_id)

    with ThreadPoolExecutor(max_workers=2) as pool:
        mutation_future = pool.submit(_mutate)
        delete_future = pool.submit(_delete)
        barrier.wait()
        mutation_result = mutation_future.result()
        assert delete_future.result()["status"] == "deleted"

    if isinstance(mutation_result, api.HTTPException):
        assert mutation_result.status_code == 404
        assert mutation_result.detail == "transient_session_not_found"
    else:
        assert mutation_result["annotation_notes"] == "racing mutation"
    with pytest.raises(api.HTTPException) as exc_info:
        api._resolve_transient_session(session_id)
    assert exc_info.value.status_code == 404
    assert not api._transient_session_state_path(session_id).exists()
...
def test_transient_delete_removes_active_and_expired_leases(monkeypatch) -> None:
    ...
    expired = api._resolve_transient_session(expired_id)
    assert expired["expires_at"] > now
    assert expired["annotation_lock"] == {}
    expired_out = api.delete_transient_dataset(expired_id)
    assert expired_out["status"] == "deleted"
