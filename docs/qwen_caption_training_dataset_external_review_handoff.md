...
- It rejects malformed review rows, unsupported actionable row origins,
  ...
  generated-QA or caption0 matches, QA ids that do not match a saved record, QA
  ids whose review-row image path does not match the stored
  caption/generated-QA record, same-id rows whose reviewed question or answer no
  longer matches the saved language record, and unresolvable synthetic caption0
  review targets before applying any imported metadata. Caption0 review rows
  can materialize a saved caption review record only when their synthetic id
