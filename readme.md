...
dataset can reuse the same trained baseline later. Candidate retention is based
on farthest-first coverage rank; raw reference-novelty distance is shown only as
a within-run diagnostic and can overlap between kept and skipped rows. The
distribution map is generated by the ingestion job itself and does not require
Class Split Dataset Analysis, which is a separate all-class image-value review.
