...
_INSTRUCTION_QA_OUTPUT_INSTRUCTION_RE = re.compile(
    r"\?\s+(?:reply|respond|respong|return|output|provide|use)\b.*$",
    re.IGNORECASE,
)
_INSTRUCTION_QA_TRAILING_OUTPUT_INSTRUCTION_RE = re.compile(
    r"\s+(?:reply|respond|respong|return|output|provide|use)\b.*$",
    re.IGNORECASE,
)


def _clean_instruction_qa_question_text(raw: Any) -> str:
    question = re.sub(r"\s+", " ", str(raw or "").strip())
    question = question.strip(" \t\r\n\"'`,;")
    if not question:
        return ""
    question = _INSTRUCTION_QA_OUTPUT_INSTRUCTION_RE.sub("?", question)
    question = _INSTRUCTION_QA_TRAILING_OUTPUT_INSTRUCTION_RE.sub("", question)
    question = re.sub(r"\s+(?:in|as)\s+json(?:\s+format)?\.?$", "", question, flags=re.IGNORECASE)
...
class SamPreloadResponse(BaseModel):
    ...
    preload_ms: Optional[float] = None
    image_cache_hit: Optional[bool] = None
...
class QwenCaptionDatasetJobRequest(BaseModel):
    ...
    openai_reasoning_effort: Optional[Literal["low", "medium", "high", "xhigh"]] = "high"
    openai_api_key_path: Optional[str] = "openAI_API_KEY_DoNotCommit"
    openai_service_tier: Optional[Literal["standard", "batch", "flex"]] = "batch"
    openai_batch_shard_size: Optional[int] = 100
    openai_batch_require_image_hash_match: Optional[bool] = True
    openai_batch_require_label_hash_match: Optional[bool] = False
    ...
    def _normalize_caption_job_input(cls, values):  # noqa: N805
        ...
        effort = str(data.get("openai_reasoning_effort") or "high").strip().lower()
        data["openai_reasoning_effort"] = effort if effort in {"low", "medium", "high", "xhigh"} else "high"
        ...
        tier = str(data.get("openai_service_tier") or "batch").strip().lower()
        data["openai_service_tier"] = tier if tier in {"standard", "batch", "flex"} else "batch"
        try:
            shard_size = int(data.get("openai_batch_shard_size") or 100)
        except (TypeError, ValueError, OverflowError):
            shard_size = 100
        data["openai_batch_shard_size"] = max(1, min(shard_size, 50_000))
        image_hash_raw = data.get("openai_batch_require_image_hash_match")
        label_hash_raw = data.get("openai_batch_require_label_hash_match")
        data["openai_batch_require_image_hash_match"] = (
            True
            if image_hash_raw is None
            else image_hash_raw is True
            or str(image_hash_raw).strip().lower() in {"1", "true", "yes", "on"}
        )
        data["openai_batch_require_label_hash_match"] = (
            False
            if label_hash_raw is None
            else label_hash_raw is True
            or str(label_hash_raw).strip().lower() in {"1", "true", "yes", "on"}
        )
        try:
        ...
        for raw_question in raw_items:
            question = _clean_instruction_qa_question_text(raw_question)
            if not question:
                continue
