...
def test_caption_dataset_job_request_defaults_openai_service_tier_to_batch() -> None:
    default_payload = QwenCaptionDatasetJobRequest(
        dataset_id="ds",
        caption_provider="openai",
        caption_request={"user_prompt": "Describe it"},
    )
    explicit_standard = QwenCaptionDatasetJobRequest(
        dataset_id="ds",
        caption_provider="openai",
        openai_service_tier="standard",
        caption_request={"user_prompt": "Describe it"},
    )
    explicit_flex = QwenCaptionDatasetJobRequest(
        dataset_id="ds",
        caption_provider="openai",
        openai_service_tier="flex",
        caption_request={"user_prompt": "Describe it"},
    )
    invalid_payload = QwenCaptionDatasetJobRequest(
        dataset_id="ds",
        caption_provider="openai",
        openai_service_tier="sync",
        caption_request={"user_prompt": "Describe it"},
    )

    assert default_payload.openai_service_tier == "batch"
    assert explicit_standard.openai_service_tier == "standard"
    assert explicit_flex.openai_service_tier == "flex"
    assert invalid_payload.openai_service_tier == "batch"
...
def test_caption_dataset_completion_ignores_imposed_questions_when_generated_qa_disabled(
    monkeypatch,
    tmp_path: Path,
) -> None:
    import localinferenceapi as api
    ...
    monkeypatch.setattr(api, "_resolve_dataset_entry", lambda dataset_id: entry)
    monkeypatch.setattr(api, "_annotation_manifest_for_entry", lambda _entry: manifest)
    monkeypatch.setattr(api, "_annotation_effective_text_label", lambda _entry, _image_relpath, _split: "A caption.")
    ...
    image = snapshot["images"][0]
    assert image["base_caption_count"] == 1
    assert image["generated_qa_count"] == 0
    assert image["missing_imposed_questions"] == []
    assert image["complete"] is True


def test_caption_dataset_completion_treats_caption0_disabled_as_zero_base_target(
    monkeypatch,
    tmp_path: Path,
) -> None:
    import localinferenceapi as api
    ...
    monkeypatch.setattr(api, "_resolve_dataset_entry", lambda dataset_id: entry)
    monkeypatch.setattr(api, "_annotation_manifest_for_entry", lambda _entry: manifest)
    monkeypatch.setattr(api, "_annotation_effective_text_label", lambda _entry, _image_relpath, _split: "")
    monkeypatch.setattr(api, "_load_dataset_caption_records", lambda _entry: [])
    monkeypatch.setattr(api, "_load_dataset_caption_instruction_records", lambda _entry: [])
    ...
    snapshot = api._qwen_caption_dataset_completion_snapshot("ds", payload=payload)
    _manifest, cases = api._qwen_caption_dataset_cases(payload, output_dir=tmp_path / "cases")

    image = snapshot["images"][0]
    assert snapshot["targets"]["base_captions_per_image"] == 0
    assert image["base_caption_deficit"] == 0
    assert image["complete"] is True
    assert cases == []
...
def test_caption_dataset_add_instruction_records_writes_caption_artifact_store(
    monkeypatch,
    tmp_path: Path,
) -> None:
    import localinferenceapi as api
    ...
    overlay_root.mkdir(parents=True)
    records_path = overlay_root / "captions" / "instruction_records.jsonl"
    store_root = tmp_path / "caption_artifact_store"
    monkeypatch.setattr(api, "CAPTION_ARTIFACT_STORE_ROOT", store_root)
    monkeypatch.setattr(api, "CAPTION_ARTIFACT_EXPORT_ROOT", store_root / "exports")
    monkeypatch.setattr(api, "_resolve_dataset_entry", lambda _dataset_id: {"id": "ds", "yolo_layout": "flat"})
        ...
        [
            {
                ...
                "answer": "A bright test image is present.",
                "answer_format": "natural",
                "validation_status": "machine_validated",
                "review_status": "unreviewed",
                "metadata": {"job_id": "job_1", "case_id": "case_a", "provider": "openai"},
            },
            {
                "id": "qa-2",
                "question": "What color is the test image?",
                "answer": "It is white.",
                "answer_format": "natural",
                "validation_status": "machine_validated",
                "review_status": "unreviewed",
                "metadata": {"job_id": "job_1", "case_id": "case_b", "provider": "openai"},
            }
        ],
        split="train",
    )

    sets = api.list_caption_artifact_sets()["sets"]
    assert added == 2
    assert records_path.exists()
    assert len(sets) == 1
    image_hash = hashlib.sha256(image_path.read_bytes()).hexdigest()
    summary = api._caption_artifact_store().summarize_images(
        [image_hash],
        caption_set_id=sets[0]["caption_set_id"],
    )
    assert summary["totals"]["qa_pair_count"] == 2
    assert summary["images"][0]["artifacts"][0]["payload"]["question"] == "What is present?"
...
def test_caption_instruction_archive_flattens_machine_validated_generated_qa(
    ...
    generated = [
        {
            ...
            "answer": "A elevated fixture stands beside the road.",
            "row_type": "generated_qa",
            "answer_source": "vlm_generated",
            "validation_status": "machine_validated",
            "review_status": "machine_validated",
    ...
    assert row["answer"] == "A elevated fixture stands beside the road."
    assert row["metadata"]["validation_status"] == "machine_validated"
...
def test_caption_dataset_materialize_instruction_artifacts_respects_run_scope(
    monkeypatch,
    tmp_path: Path,
) -> None:
    import localinferenceapi as api
    ...
    for name in ("keep.jpg", "skip.jpg"):
        Image.new("RGB", (16, 16)).save(image_dir / name)
        (label_dir / Path(name).with_suffix(".txt")).write_text("0 0.5 0.5 0.25 0.25\n", encoding="utf-8")
    entry = {
        "id": "ds",
        "label": "Dataset",
        "yolo_layout": "split",
    ...
    collect_rows = [
        {
            **row,
            "image_path": str(image_dir / str(row["image_name"])),
        }
    ...
    monkeypatch.setattr(api, "_dataset_effective_root_from_entry", lambda _entry: dataset_root)
    monkeypatch.setattr(api, "_annotation_manifest_for_entry", lambda _entry: manifest)
    monkeypatch.setattr(api, "_annotation_collect_images", lambda _entry: list(collect_rows))
    monkeypatch.setattr(api, "_load_dataset_caption_records", lambda _entry: [])
    monkeypatch.setattr(api, "_load_dataset_caption_instruction_records", lambda _entry: [])
    monkeypatch.setattr(
        api,
    ...
    monkeypatch.setattr(api, "_annotation_effective_text_label", lambda *_args, **_kwargs: "")
    ...
    result = api._qwen_caption_dataset_materialize_instruction_artifacts(
        ...
        payload=QwenCaptionDatasetJobRequest(
            ...
            include_generated_qa_in_training=False,
            include_deterministic_metadata_qa=True,
            image_names=["keep.jpg"],
            max_images=1,
        ),
        artifact_dir=tmp_path / "scoped" / "instruction_artifacts",
    )

    report = json.loads(Path(result["report_json"]).read_text())
    assert result["status"] == "ok"
    assert report["image_count"] == 1
    assert list(report["qa_count_per_image"]) == ["keep.jpg"]
    with zipfile.ZipFile(result["bundle_zip"]) as zf:
        names = set(zf.namelist())
    assert "images/train/keep.jpg" in names
    assert "labels/train/keep.txt" in names
    assert "images/train/skip.jpg" not in names
    assert "labels/train/skip.txt" not in names


def test_caption_export_empty_selected_scope_does_not_export_full_instruction_dataset(
    monkeypatch,
    tmp_path: Path,
) -> None:
    import localinferenceapi as api
    ...
    image_dir.mkdir(parents=True)
    Image.new("RGB", (16, 16)).save(image_dir / "keep.jpg")
    entry = {
        "id": "ds",
        "label": "Dataset",
        "yolo_layout": "flat",
    ...
    result = api.export_captions(
        "ds",
        {
            "selected_image_names": ["missing.jpg"],
            "include_caption0_in_training": False,
            "include_generated_qa_in_training": False,
            "include_deterministic_metadata_qa": True,
        },
    ...
    assert result["instruction_report"]["image_count"] == 0
    assert result["instruction_summary"]["instruction_training_row_count"] == 0


def test_caption_export_flat_scope_keeps_split_prefixed_caption_and_qa_records(
    monkeypatch,
    tmp_path: Path,
) -> None:
    import localinferenceapi as api
    ...
    caption_records = [
        {
            "id": "caption-1",
            "image_name": "train/keep.jpg",
            "image_key": "train/keep.jpg",
            "split": "train",
            "caption": "A top-down view shows one building.",
            "is_primary": True,
            "source": "qwen_caption_job",
            "lifecycle_status": "active",
        },
        {
            "id": "caption-2",
            "image_name": "train/skip.jpg",
            "image_key": "train/skip.jpg",
            "split": "train",
            "caption": "A top-down view shows another building.",
            "is_primary": True,
            "source": "qwen_caption_job",
            "lifecycle_status": "active",
        },
    ...
    monkeypatch.setattr(api, "_annotation_manifest_for_entry", lambda _entry: manifest)
    monkeypatch.setattr(api, "_annotation_collect_images", lambda _entry: list(collect_rows))
    monkeypatch.setattr(api, "_load_dataset_caption_records", lambda _entry: list(caption_records))
    monkeypatch.setattr(api, "_load_dataset_caption_instruction_records", lambda _entry: list(instruction_records))
    monkeypatch.setattr(api, "_annotation_effective_text_label", lambda *_args, **_kwargs: "")

    result = api.export_captions(
        "ds",
        {
            "selected_image_names": ["keep.jpg"],
            "include_caption0_in_training": True,
            "include_generated_qa_in_training": True,
            "include_deterministic_metadata_qa": False,
        },
    ...
    image_paths = {str(row.get("image_path") or "") for row in result["instruction_training_rows"]}
    assert result["summary"]["caption_count"] == 1
    assert result["instruction_report"]["image_count"] == 1
    assert image_paths == {"keep.jpg"}
    assert result["instruction_summary"]["generated_qa_pair_count"] == 1
    assert all("skip.jpg" not in json.dumps(row) for row in result["instruction_archive_rows"])
...
def test_caption_dataset_job_persistence_mirrors_custom_output_for_restart_discovery(
    ...
    assert (
        custom_output_dir / api.QWEN_CAPTION_DATASET_DISCOVERY_FILENAME
    ).exists()
    assert (
        jobs_root
        / "qcap_custom_output"
        / api.QWEN_CAPTION_DATASET_DISCOVERY_FILENAME
    ).exists()
...
def test_caption_dataset_discovery_summary_avoids_reloading_large_job_payload(
    monkeypatch,
    tmp_path: Path,
) -> None:
    import localinferenceapi as api
    ...
    payload = {
        ...
        "request": {
            ...
            "max_auto_resumes": 2,
            "output_dir": str(artifact_dir),
            "large_prompt_state": "x" * 100_000,
        },
        "result": {
            "output_dir": str(artifact_dir),
            "processed": 5,
            "runner_summary": {"large": "y" * 500_000},
        },
    }
    (job_dir / "job.json").write_text(json.dumps(payload))
    monkeypatch.setattr(api, "QWEN_CAPTION_DATASET_JOB_ROOT", jobs_root)

    first = api._qwen_caption_dataset_load_discovery_summary(job_dir)

    assert first is not None
    assert first["job_id"] == "qcap_large"
    assert first["request"]["set_and_forget"] is True
    assert first["result"]["processed"] == 5
    assert "large_prompt_state" not in first["request"]
    assert "runner_summary" not in first["result"]
    sidecar = job_dir / api.QWEN_CAPTION_DATASET_DISCOVERY_FILENAME
    assert sidecar.stat().st_size < 2_000

    monkeypatch.setattr(
        api,
        "_load_qwen_caption_dataset_job_payload",
        lambda _job_id: (_ for _ in ()).throw(
            AssertionError("valid sidecar must bypass full job JSON")
        ),
    )
    second = api._qwen_caption_dataset_load_discovery_summary(job_dir)

    assert second == first


def test_caption_dataset_auto_resume_does_not_load_terminal_job_payloads(
    monkeypatch,
) -> None:
    import localinferenceapi as api
    ...
    summaries = [
        {
            "job_id": f"qcap_completed_{idx}",
            "status": "completed",
            "output_dir": f"/tmp/completed-{idx}",
            "updated_at": float(idx),
            "request": {"set_and_forget": True},
            "result": {},
        }
    ...
    monkeypatch.setattr(
        api,
        "_iter_persisted_qwen_caption_dataset_job_summaries",
        lambda: iter(summaries),
    )
    monkeypatch.setattr(api, "QWEN_CAPTION_DATASET_JOBS", {})
    monkeypatch.setattr(
        api,
        "_load_qwen_caption_dataset_job_payload",
        lambda _job_id: (_ for _ in ()).throw(
            AssertionError("terminal summaries must not load full payloads")
        ),
    )
