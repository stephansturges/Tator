...
def test_caption_prompt_rejects_malformed_glossary_terms():
    prompt, counts, used, truncated = _build_qwen_caption_prompt(
        ...
        [
            ...
            QwenCaptionHint(label="ElevatedFixture", bbox=[10, 50, 20, 95], confidence=0.7),
        ],
        image_width=100,
        image_height=100,
        include_counts=True,
        ...
        detailed_mode=True,
        restrict_to_labels=True,
        labelmap_glossary='Building: [\nSolarpanels: solar panels and rooftop arrays\nElevatedFixture: elevated fixture or antenna',
    )

    assert counts == {"Building": 1, "Solarpanels": 1, "ElevatedFixture": 1}
    assert used == 3
    assert truncated is False
    ...
    assert "elevated fixture: 1" in prompt
...
def test_caption_count_conflicts_catch_pluralized_singletons_with_glossary_terms():
    glossary = {"PoleFixture": ["elevated fixture"]}
    caption = (
        "A tall elevated fixture stands near the center-left. "
        "A few elevated fixtures are visible throughout the neighborhood."
    )
    ...
    assert (
        _caption_count_conflicts(
            "A elevated fixture stands near the road, with another pole farther back.",
            {"PoleFixture": 2},
            glossary,
        )
        == []
...
def test_default_caption_glossary_matches_camelcase_labelmap_names():
    glossary = _default_agent_glossary_for_labelmap(["SmallVehicle", "PoleFixture", "StorageTank", "ElevatedFixture"])

    assert '"SmallVehicle": [\n    "Small Vehicle"' in glossary
    assert '"PoleFixture": [\n    "Pole Fixture"' in glossary
    assert '"StorageTank": [\n    "Storage Tank"' in glossary
    assert '"ElevatedFixture": [\n    "ElevatedFixture"' in glossary
...
def test_caption_request_accepts_json_object_glossary_template():
    ...
    assert '"ElevatedFixture"' in payload.labelmap_glossary
    assert "elevated fixture" in payload.labelmap_glossary
...
def test_qwen_caption_prompt_preview_hardens_windowed_evidence_contract(monkeypatch):
    ...
    assert "elevated fixture" in preview.full_text
