...
if [[ "$SKIP_GPU" == "1" ]]; then
  "${PYTHON_BIN}" tools/fuzz_tier1.py --base-url "$BASE_URL" --fuzz-pack "$FUZZ_PACK" --skip-gpu --out "$OUT_DIR/tier1.json"
else
  "${PYTHON_BIN}" tools/fuzz_tier1.py --base-url "$BASE_URL" --fuzz-pack "$FUZZ_PACK" --out "$OUT_DIR/tier1.json"
fi
