...
def normalize_embedding_aggregation(value: Any) -> str:
    ...
    if mode in {
        "sam3_mask_salad_fusion_v1",
        "sam3_salad_fusion",
        "dinov3_sam3_salad_fusion",
    }:
        return "sam3_mask_salad_fusion_v1"
    if mode in {
        "sam3_mask_fusion_v1",
        "sam3_mask_fusion",
        "dinov3_sam3_mask_fusion",
    }:
        return "sam3_mask_fusion_v1"
...
def preprocess_crop_with_metadata(
    crop: Image.Image,
    *,
    mode: str,
    canonical_size: int,
) -> Tuple[Image.Image, Dict[str, Any]]:
    """Preprocess a crop and describe where real pixels land in the result.

    Spatial-token consumers need this transform metadata to distinguish image
    content from center padding and to map patch cells back to source pixels.
    Keeping it beside the image transform prevents the pooled and unpooled
    pipelines from silently drifting apart.
    """
    if normalize_preprocess_mode(mode) == "native":
        processed = crop.convert("RGB").copy()
    ...
    if width == height:
        processed = rgb.resize((size, size), Image.Resampling.LANCZOS)
        return processed, {
            "preprocess_mode": "canonical",
            "input_size": [int(size), int(size)],
            "content_rect": [0, 0, int(size), int(size)],
        }
    try:
    ...
    processed = canvas.resize((size, size), Image.Resampling.LANCZOS)
    scale = float(size) / float(side)
    content_left = int(round(offset_x * scale))
    content_top = int(round(offset_y * scale))
    content_right = int(round((offset_x + width) * scale))
    content_bottom = int(round((offset_y + height) * scale))
    return processed, {
        "preprocess_mode": "canonical",
        "input_size": [int(size), int(size)],
        "content_rect": [
            max(0, min(int(size), content_left)),
            max(0, min(int(size), content_top)),
            max(0, min(int(size), content_right)),
            max(0, min(int(size), content_bottom)),
        ],
    }


def preprocess_crop(crop: Image.Image, *, mode: str, canonical_size: int) -> Image.Image:
    processed, _metadata = preprocess_crop_with_metadata(
        crop,
        mode=mode,
    ...
    for view_crop_mode, view_padding, view_name in embedding_view_specs(
        ...
        try:
            ...
            try:
                ...
                pass
        try:
            processed, preprocess_metadata = preprocess_crop_with_metadata(
                masked,
        ...
        metadata.append(
            {
                ...
                "source_crop_xyxy": [int(v) for v in bounds],
                "source_crop_size": [
                    max(0, int(bounds[2] - bounds[0])),
                    max(0, int(bounds[3] - bounds[1])),
                ],
                "processed_input_size": list(preprocess_metadata["input_size"]),
                "processed_content_rect": list(preprocess_metadata["content_rect"]),
            }
        )
    if primary_bounds is None:
