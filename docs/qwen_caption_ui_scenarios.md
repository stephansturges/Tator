...
record, or whose image path still matches but whose reviewed text is no longer
current, so mixed valid/stale packets do not partially apply. The browser
...
missing stable QA ids, unresolved image context, and caption0 rows that would
create a saved caption without a synthetic id matching the selected dataset,
