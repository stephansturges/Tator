...
def test_build_falcon_query_tiers_uses_naturalized_labels_without_static_synonyms() -> None:
    rows = _flat_falcon_query_rows(
        ["light_vehicle", "elevated_fixture", "person"],
        labelmap=["light_vehicle", "elevated_fixture", "person"],
        glossary="",
    )
    assert rows == [
        {"class_name": "light_vehicle", "query": "light vehicle", "term": "light vehicle"},
        {"class_name": "elevated_fixture", "query": "elevated fixture", "term": "elevated fixture"},
...
def test_build_falcon_query_tiers_keeps_canonical_noun_in_tier_a_when_usable() -> None:
    tiers = build_falcon_query_tiers(
        ["boat", "building", "elevated_fixture"],
        labelmap=["boat", "building", "elevated_fixture"],
        glossary='{"boat":["canoe","ship"],"building":["house","warehouse"],"elevated_fixture":["streetlight","mast"]}',
    )
    ...
    assert tiers["elevated_fixture"][0]["query"] == "elevated fixture"


def test_build_falcon_query_tiers_can_force_canonical_only_style() -> None:
    tiers = build_falcon_query_tiers(
        ["light_vehicle", "elevated_fixture"],
        labelmap=["light_vehicle", "elevated_fixture"],
        glossary='{"light_vehicle":["car","SUV"],"elevated_fixture":["streetlight","mast"]}',
    ...
    assert tiers["elevated_fixture"] == [
        {
            "class_name": "elevated_fixture",
            "query": "elevated fixture",
            "term": "elevated fixture",
            "tier": "A",
        }
    ]
