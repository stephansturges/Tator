...
def _default_agent_glossary_for_labelmap(labelmap: Sequence[str]) -> str:
    ...
    for label in labelmap:
        ...
        if natural:
            ...
            if "gas" in label_key and "tank" in label_key and "gas tank" not in {term.lower() for term in terms}:
                terms.append("gas tank")
            if "tank" in natural_key and "storage tank" not in {term.lower() for term in terms}:
                terms.append("storage tank")
            mapped[label] = _dedupe_synonyms(terms)
    if not mapped:
        return ""
