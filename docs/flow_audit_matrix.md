...
| DS-002 | Dataset delete + restore | `DELETE /datasets/{dataset_id}`, `/datasets/trash*` | `api/datasets.py -> delete_dataset_entry`, `restore_dataset_trash_entry` | cpu_clean | not_required | fixed | Managed deletes now soft-delete to backend trash with restore; linked deletes still remove only the registry record/overlays. |
