...
- The caption API now accepts reviewed instruction JSONL and applies only review
  ...
  decisions must carry an embedded dataset id, and rows carrying a missing or
  different dataset id are blocked by the UI import preflight and rejected by
  the backend before any review metadata is applied. Rows carrying a known QA id
  but a mismatched image path are also rejected before any metadata is applied.
  Rows carrying the same QA id and image context are still rejected if their
  reviewed question, candidate answer, or training answer no longer matches the
  saved caption or generated-QA record.
