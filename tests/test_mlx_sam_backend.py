...
def test_build_mlx_sam_predictor_loads_external_package_without_name_collision(tmp_path, monkeypatch):
    ...
    second_predictor = mlx_sam.build_mlx_sam_predictor()
    assert second_predictor.predictor is not predictor.predictor
    assert second_predictor.predictor.model is predictor.predictor.model
...
def test_mlx_preload_materializes_image_features(monkeypatch):
    calls = []
    ...
    class DummyPredictor:
        ...
        def set_image(self, image):
            self.features = ("features", image.shape)

    monkeypatch.setattr(mlx_sam, "_evaluate_mlx_values", lambda *values: calls.append(values))
    adapter = mlx_sam.MlxSamPredictorAdapter(DummyPredictor())

    adapter.set_image(np.zeros((5, 7, 3), dtype=np.uint8))

    assert calls == [(adapter.predictor.features,)]
