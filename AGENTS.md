...
- When enabling SAM3, request access to the [facebook/sam3](https://huggingface.co/facebook/sam3) repo, upgrade to a transformer build that includes `Sam3Model/Sam3Processor`, and run `hf auth login` (after creating a read token) so Hugging Face can stream the gated checkpoints automatically before switching the UI selector to “SAM 3”.
