...
RUN_COMPONENT_PATTERN = re.compile(r"[^A-Za-z0-9._-]+")
api: Any = None
...
def _safe_run_component(value: Any) -> str:
    ...
    safe = RUN_COMPONENT_PATTERN.sub("_", raw).strip("._-")
    if not safe:
        safe = "scope"
    if safe != raw or len(safe) > 80:
        digest = hashlib.sha256(raw.encode("utf-8")).hexdigest()[:8]
        safe = f"{safe[:71]}_{digest}"
    return safe


def _run_id(variant: str, class_name: str) -> str:
    return f"{_safe_run_component(variant)}_{_safe_run_component(class_name)}"
...
def _load_zip_labels(path: Path) -> Dict[str, List[str]]:
    ...
    with zipfile.ZipFile(path) as zf:
        for info in zf.infolist():
            ...
            stem = Path(name).stem
            if stem in labels:
                raise ValueError(f"duplicate label basename in archive: {stem}")
...
def _build_manifest(*, image_dir: Path, label_zip: Path, labelmap: List[str]) -> Dict[str, Any]:
    ...
    class_counts = {class_name: 0 for class_name in labelmap}
    class_bbox_areas: Dict[str, List[float]] = {class_name: [] for class_name in labelmap}
    ...
    for image_path in sorted(image_dir.rglob("*")):
        ...
        if image_path.stem in image_stems:
            raise ValueError(
                f"flat YOLO layout requires unique image basenames: {image_path.stem}"
            )
        ...
        for line in label_lines:
            ...
            try:
                ...
                if class_index < 0 or class_index >= len(labelmap):
                    raise ValueError("class index out of range")
                if width <= 0 or height <= 0:
                    raise ValueError("non-positive bbox")
            except (IndexError, TypeError, ValueError):
                malformed_label_lines += 1
                continue
            class_name = labelmap[class_index]
            class_counts[class_name] += 1
            class_bbox_areas[class_name].append(width * height)
        rows.append(
            {
                "split": "train",
                "image_relpath": rel,
    ...
    missing_images = sorted(set(labels_by_stem) - image_stems)
    bbox_area_medians = {
        class_name: float(statistics.median(values))
        for class_name, values in class_bbox_areas.items()
        if values
    }
    return {
        ...
        "dataset_stats": {
            ...
            "label_file_count": len(labels_by_stem),
            "non_empty_label_file_count": non_empty_label_files,
            "empty_label_file_count": max(0, len(labels_by_stem) - non_empty_label_files),
            "total_object_count": int(sum(class_counts.values())),
            "missing_images_for_label_files": missing_images,
            "malformed_label_line_count": malformed_label_lines,
            "class_counts": class_counts,
            "class_bbox_area_medians": bbox_area_medians,
        },
    }
...
def _minimum_matrix(sample_cap: int, classes: Sequence[str]) -> List[Dict[str, Any]]:
    ...
    variants = {
        ...
        "B4": {"preprocess_mode": "canonical", "embedding_adjustment": "remove_size_bias", "projection": "umap", "projection_neighbor_k": 15},
        "B5": {"preprocess_mode": "canonical", "embedding_adjustment": "remove_size_bias", "projection": "umap", "projection_neighbor_k": 50},
        "B6": {"preprocess_mode": "canonical", "embedding_adjustment": "remove_size_bias", "projection": "umap", "projection_neighbor_k": 0},
    }
    ...
    for class_name, scope in scopes:
        for variant_id, settings in variants.items():
            ...
            run["run_id"] = _run_id(variant_id, class_name)
            runs.append(run)
    return runs
...
def _json_safe(value: Any) -> Any:
    return _get_api()._class_analysis_json_safe(value)
...
def _apply_embedding_postprocess(
    ...
    arr = api_module._class_analysis_normalize_rows(np.asarray(embeddings, dtype=np.float32))
    ...
    if mode_norm == "remove_image_bias":
        ...
        for idxs in groups.values():
            ...
            applied_groups += 1
        return api_module._class_analysis_normalize_rows(adjusted), {
            "mode": mode_norm,
    ...
    if mode_norm in {"pca64", "whiten64"}:
        ...
        return api_module._class_analysis_normalize_rows(reduced), {
            "mode": mode_norm,
            "applied": True,
...
def _run_one(
    ...
    *,
    ...
    out_dir = (output_root_resolved / run_id).resolve()
    if out_dir.parent != output_root_resolved:
        raise ValueError(f"unsafe run_id output path: {run_id}")
    ...
    required_artifacts = [
        ...
        out_dir / "review_queue.jsonl",
    ]
    if not force and all(path.is_file() for path in required_artifacts):
    ...
    job = api_module.ClassAnalysisJob(job_id=run_id, request=request)
    start = time.perf_counter()
    records, crops, summary = api_module._class_analysis_collect_records(job.request, job=job, out_dir=out_dir)
    ...
    feats = api_module._class_analysis_encode_crops(
        crops,
        job=job,
        head={
    ...
    raw_embeddings = api_module._class_analysis_normalize_rows(np.asarray(feats, dtype=np.float32))
    embeddings, adjustment_info = api_module._class_analysis_apply_embedding_adjustment(
        raw_embeddings,
        records,
    ...
    result = api_module._class_analysis_build_result(
        records,
        embeddings,
        summary={
            **summary,
        ...
        projection_mode=str(run.get("projection_mode") or api_module.CLASS_ANALYSIS_DEFAULT_PCA_PROJECTION_MODE),
        projection_neighbor_k=int(
            run.get("projection_neighbor_k")
            if run.get("projection_neighbor_k") is not None
            else 50
        ),
        projection_preprocess=str(run.get("projection_preprocess") or "none"),
        projection_min_dist=float(run.get("projection_min_dist") if run.get("projection_min_dist") is not None else 0.08),
        projection_metric=str(run.get("projection_metric") or "cosine"),
        projection_spread=float(run.get("projection_spread") if run.get("projection_spread") is not None else 1.0),
    ...
    with (out_dir / "review_queue.jsonl").open("w", encoding="utf-8") as handle:
        for candidate in result.get("wrong_class_candidates") or []:
            if isinstance(candidate, dict):
                handle.write(
                    json.dumps(
                        _json_safe({**candidate, "manual_review_status": "pending"}),
                        ensure_ascii=False,
                    )
                    + "\n"
                )
...
def _metrics_from_result(run_id: str, run: Dict[str, Any], result: Dict[str, Any], elapsed: float) -> Dict[str, Any]:
    ...
    projection_quality = diagnostics.get("projection_quality") or {}
    return {
        "run_id": run_id,
        ...
        "projection_mode": summary.get("projection_mode") or run.get("projection_mode"),
        "projection_preprocess": summary.get("projection_preprocess") or run.get("projection_preprocess"),
        "projection_neighbor_k": summary.get("projection_neighbor_k"),
        "projection_neighbor_k_resolved": summary.get("projection_neighbor_k_resolved"),
        "projection_min_dist": summary.get("projection_min_dist"),
        "projection_metric": summary.get("projection_metric"),
        "projection_spread": summary.get("projection_spread"),
        "projection_trustworthiness": projection_quality.get("trustworthiness"),
        "projection_class_silhouette": projection_quality.get("class_silhouette"),
        "projection_class_separation_ratio": projection_quality.get("class_separation_ratio"),
        "projection_separation_score": projection_quality.get("separation_score"),
        "projection_overview_score": projection_quality.get("overview_score"),
        "projection_recommendation": projection_quality.get("projection_recommendation"),
        "projection_recommended_mode": summary.get("projection_recommended_mode"),
        ...
        "manual_accepted_wrong_class_rate": None,
    }
...
def _write_leaderboard(output_root: Path, metrics_rows: List[Dict[str, Any]]) -> None:
    ...
    fields = [
        ...
        "projection",
        "projection_mode",
        "projection_preprocess",
        "projection_neighbor_k",
        "projection_neighbor_k_resolved",
        "projection_min_dist",
        "projection_metric",
        "projection_spread",
        "projection_trustworthiness",
        "projection_class_silhouette",
        "projection_class_separation_ratio",
        "projection_separation_score",
        "projection_overview_score",
        "projection_recommendation",
        "projection_recommended_mode",
        "object_count",
        "raw_object_count",
        "sample_cap",
        "cache_hit_rate",
        ...
        "manual_accepted_wrong_class_rate",
    ]
    for csv_name in ("leaderboard.csv", "metrics.csv"):
        with (output_root / csv_name).open("w", newline="", encoding="utf-8") as handle:
    ...
    lines.append("")
    lines.append(
        "Human usefulness remains pending until review_queue.jsonl rows are visually audited; "
        "automated graph metrics are not a substitute for that review."
    )
    lines.append("")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--label-zip", type=Path, default=None)
...
def _selected_classes(
    ...
    count = max(0, count)
    stats = manifest.get("dataset_stats") if isinstance(manifest, dict) else None
    class_counts = stats.get("class_counts") if isinstance(stats, dict) else None
    bbox_medians = stats.get("class_bbox_area_medians") if isinstance(stats, dict) else None
    populated = [
        str(class_name)
        for class_name in labelmap
        if str(class_name or "").strip()
        and isinstance(class_counts, dict)
        and int(class_counts.get(str(class_name)) or 0) > 0
    ]
    if not populated:
        return [str(class_name) for class_name in labelmap[:count] if str(class_name or "").strip()]
    selected: List[str] = []

    def add(class_name: str) -> None:
        if class_name and class_name not in selected and len(selected) < count:
            selected.append(class_name)

    add(max(populated, key=lambda name: (int(class_counts.get(name) or 0), name)))
    if isinstance(bbox_medians, dict):
        sized = [name for name in populated if bbox_medians.get(name) is not None]
        if sized:
            add(min(sized, key=lambda name: (float(bbox_medians[name]), name)))
    add(min(populated, key=lambda name: (int(class_counts.get(name) or 0), name)))
    for class_name in sorted(
        populated,
        key=lambda name: (-int(class_counts.get(name) or 0), name),
    ):
        add(class_name)
    return selected


def _resolve_dataset_paths(args: argparse.Namespace) -> Tuple[Path, Path, Path, Path]:
    dataset_root = args.dataset_root.expanduser().resolve()
    label_zip = (args.label_zip or (dataset_root / "labels.zip")).expanduser().resolve()
    labelmap = (args.labelmap or (dataset_root / "labelmap.txt")).expanduser().resolve()
    image_dir = (args.image_dir or (dataset_root / "images")).expanduser().resolve()
    if not dataset_root.is_dir():
        raise SystemExit(f"--dataset-root is not a directory: {dataset_root}")
    if not label_zip.is_file():
        raise SystemExit(f"label archive not found: {label_zip}")
    if not labelmap.is_file():
        raise SystemExit(f"labelmap not found: {labelmap}")
    if not image_dir.is_dir():
        raise SystemExit(f"image directory not found: {image_dir}")
    return dataset_root, label_zip, labelmap, image_dir


def _validate_unique_run_ids(runs: Sequence[Dict[str, Any]]) -> None:
    run_ids = [str(run.get("run_id") or "") for run in runs]
    duplicates = sorted({run_id for run_id in run_ids if run_ids.count(run_id) > 1})
    if duplicates:
        raise ValueError(f"duplicate experiment run ids: {', '.join(duplicates)}")
...
def _stage_workspace(
    *,
    ...
    api_module = _get_api()
    class_root = Path(api_module.CLASS_ANALYSIS_ROOT).expanduser().resolve()
    output_root_resolved = output_root.expanduser().resolve()
    try:
        output_root_resolved.relative_to(class_root)
    except ValueError as exc:
        raise ValueError(
            f"--output-root must be inside Class Analysis storage: {class_root}"
        ) from exc
    workspace_dir = output_root_resolved / "_workspace"
    staged_images = workspace_dir / "images"
    staged_images.mkdir(parents=True, exist_ok=True)
    image_root = image_dir.expanduser().resolve()
    for row in manifest.get("images") or []:
        rel = Path(str(row.get("image_relpath") or ""))
        source = (image_root / rel).resolve(strict=True)
        try:
            source.relative_to(image_root)
        except ValueError as exc:
            raise ValueError(f"manifest image escapes image root: {rel}") from exc
        destination = staged_images / rel
        destination.parent.mkdir(parents=True, exist_ok=True)
        if destination.exists():
            if destination.is_file() and destination.stat().st_size == source.stat().st_size:
                continue
            raise ValueError(f"staged image conflicts with source: {rel}")
        try:
...
def main() -> None:
    args = parse_args()
    dataset_root, label_zip, labelmap_path, image_dir = _resolve_dataset_paths(args)
    ...
    selected_classes = _selected_classes(args, labelmap, manifest)
    ...
    workspace_dir = _stage_workspace(
        manifest=manifest,
        image_dir=image_dir,
