...
def test_parse_api_error_formats_caption_job_guards_for_operators():
    ...
    script = "\n".join(
        [
            ...
            "  ['caption_read_busy:qcap_4:running', 'Caption archive reads are blocked while caption dataset job qcap_4 is running. Wait for that job to finish, then retry.'],",
...
def test_qwen_caption_instruction_action_failures_update_export_health_without_double_prefix():
    ...
    script = "\n".join(
        [
            ...
            "  'Instruction archive export failed: Caption export is blocked while caption dataset job q1 is running.'",
            ");",
