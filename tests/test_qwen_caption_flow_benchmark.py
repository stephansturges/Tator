...
def test_instruction_qa_prompt_uses_canonical_glossary_terms_for_classes(tmp_path: Path) -> None:
    ...
    request_json.write_text(json.dumps({"labelmap_glossary": {"ElevatedFixture": ["elevated fixture", "power pole"]}}))
    args = _parent_args(
        tmp_path,
        tmp_path / "out",
    ...
    prompt_spec = bench.build_instruction_qa_prompt(
        {"case_id": "case-elevatedfixture", "class_counts": {"ElevatedFixture": 1}},
        "A elevated fixture stands beside a road.",
        args,
    )

    assert prompt_spec["source_context"]["object_counts"] == {"elevated fixture": 1}
    assert '"canonical_term": "elevated fixture"' in prompt_spec["prompt"]
...
def test_instruction_qa_verifier_rewrites_raw_label_leaks(tmp_path: Path) -> None:
                ...
                json.dumps(
                    {
                        "qa_pairs": [
                            {
                                "question": "What is beside the ElevatedFixture?",
                                "answer": "A road is likely beside the ElevatedFixture.",
                            }
                        ]
                    }
                ),
                ...
                json.dumps(
                    {
                        "qa_pairs": [
                            {
                                "qa_id": "case-elevatedfixture__generated_qa_0001",
                                "decision": "rewrite",
                                "question": "What is beside the elevated fixture?",
                                "answer": "A road is beside the elevated fixture.",
                                "rejection_reasons": [],
                            }
                        ]
                    }
    ...
    assert pair["question"] == "What is beside the elevated fixture?"
    assert pair["answer"] == "A road is beside the elevated fixture."
    assert pair["metadata"]["original_question"] == "What is beside the ElevatedFixture?"
...
def test_instruction_qa_verifier_rejects_failed_rewrites(tmp_path: Path) -> None:
    ...
    result = bench.generate_instruction_qa_pairs(
        FakeApi(),
        {"case_id": "case-bad", "class_counts": {"ElevatedFixture": 1}},
        image_path,
        "A elevated fixture stands beside a road.",
        _parent_args(
            tmp_path,
...
def test_caption_quality_accepts_glossary_canonical_label_mentions() -> None:
    quality = bench.summarize_caption(
        "The scene contains 1 elevated fixture beside the road.",
        {"ElevatedFixture": 1},
        {"ElevatedFixture": ["elevated fixture", "power pole"]},
    )
