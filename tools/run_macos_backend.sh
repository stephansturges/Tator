...
MLX_SAM_CACHE_ROOT="${TATOR_MLX_SAM_CACHE_ROOT:-${ROOT_DIR}/.cache/tator/mlx-sam}"
MLX_SAM_MODEL_DEFAULT="${MLX_SAM_CACHE_ROOT}/models/sam-vit-base"
MLX_SAM_SOURCE_DEFAULT="${MLX_SAM_CACHE_ROOT}/mlx-examples/segment_anything"
MLX_SAM3_REVISION_DEFAULT="${SAM3_MLX_MODEL_REVISION:-dfe573c3171dbcfda8399c650d9135afa7e94592}"
MLX_SAM3_MODEL_DEFAULT="${HF_HOME:-${HOME}/.cache/huggingface}/hub/models--mlx-community--sam3-bf16/snapshots/${MLX_SAM3_REVISION_DEFAULT}"
if [[ -z "${SAM_MLX_MODEL_PATH:-}" && -f "${MLX_SAM_MODEL_DEFAULT}/config.json" && -f "${MLX_SAM_MODEL_DEFAULT}/model.safetensors" ]]; then
  export SAM_MLX_MODEL_PATH="${MLX_SAM_MODEL_DEFAULT}"
fi
if [[ -z "${SAM_MLX_ROOT:-}" && -f "${MLX_SAM_SOURCE_DEFAULT}/segment_anything/sam.py" && -f "${MLX_SAM_SOURCE_DEFAULT}/segment_anything/predictor.py" ]]; then
  export SAM_MLX_ROOT="${MLX_SAM_SOURCE_DEFAULT}"
fi
if [[ -z "${SAM3_MLX_MODEL_PATH:-}" && -f "${MLX_SAM3_MODEL_DEFAULT}/config.json" ]] && [[ -f "${MLX_SAM3_MODEL_DEFAULT}/model.safetensors" || -f "${MLX_SAM3_MODEL_DEFAULT}/model.safetensors.index.json" ]]; then
  export SAM3_MLX_MODEL_PATH="${MLX_SAM3_MODEL_DEFAULT}"
fi
...
export SAM3_BACKEND="${SAM3_BACKEND:-auto}"
export SAM1_BACKEND="${SAM1_BACKEND:-auto}"
export SAM_DECODER_WARMUP="${SAM_DECODER_WARMUP:-1}"
# A slot owns mutable image-embedding state. One slot avoids background Metal
# contention on a laptop; operators can explicitly raise the environment value.
export SAM_PREDICTOR_SLOTS="${SAM_PREDICTOR_SLOTS:-1}"
export QWEN_DEVICE="${QWEN_DEVICE:-auto}"
export TATOR_MTPLX_EXECUTABLE="${TATOR_MTPLX_EXECUTABLE:-${ROOT_DIR}/.venv-mtplx/bin/mtplx}"
export TATOR_MTPLX_HOST="${TATOR_MTPLX_HOST:-127.0.0.1}"
export TATOR_MTPLX_PORT="${TATOR_MTPLX_PORT:-18081}"
...
if [[ "${SAM1_BACKEND}" != "torch" && ( -z "${SAM_MLX_MODEL_PATH:-}" || -z "${SAM_MLX_ROOT:-}" ) ]]; then
  echo "SAM 1 MLX assets are not configured; interactive SAM will fall back to Torch." >&2
  echo "Install the local fast path with: .venv-macos/bin/python tools/setup_mlx_sam.py" >&2
fi
if [[ "${SAM3_BACKEND}" != "torch" && -z "${SAM3_MLX_MODEL_PATH:-}" ]]; then
  echo "SAM 3 MLX assets are not configured; SAM3 auto mode will fall back to Torch/MPS." >&2
  echo "Install the pinned MLX SAM3 checkpoint with: .venv-macos/bin/python tools/setup_mlx_sam3.py" >&2
fi
