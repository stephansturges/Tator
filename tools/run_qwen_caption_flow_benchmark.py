...
DEFAULT_OPENAI_SERVICE_TIER = "batch"
OPENAI_SERVICE_TIER_CHOICES = ("standard", "batch", "flex")
...
def summarize_qwen_caption_io_event_lines(lines: Sequence[str], *, path: str = "") -> dict[str, Any]:
    ...
    for line in lines:
        ...
        if isinstance(usage, Mapping):
            ...
            cached_tokens = _usage_int(input_details, "cached_tokens") or _usage_int(usage, "cached_input_tokens")
            reasoning_tokens = _usage_int(output_details, "reasoning_tokens") or _usage_int(usage, "reasoning_tokens")
            if input_tokens or output_tokens or total_tokens:
                ...
                usage_total["reasoning_tokens"] += reasoning_tokens
    summary = {
        "path": str(path),
        "rows": rows,
    ...
    if usage_rows:
        summary["usage_rows"] = usage_rows
        summary["usage"] = usage_total
...
class OpenAICaptionApiAdapter:
    ...
    def __init__(self, local_api: Any, args: argparse.Namespace):
        ...
        service_tier = str(
            getattr(args, "openai_service_tier", DEFAULT_OPENAI_SERVICE_TIER)
            or DEFAULT_OPENAI_SERVICE_TIER
        ).strip().lower()
        self.service_tier = service_tier if service_tier in OPENAI_SERVICE_TIER_CHOICES else DEFAULT_OPENAI_SERVICE_TIER
    ...
    def _request_responses_api(
        ...
        *,
        ...
        self._last_response_usage = dict(usage or {})
        self._last_response_id = str(data.get("id") or "")
        self._last_response_headers = dict(response_headers or {})
        self._set_progress(
            active=False,
            ...
            event={
                ...
                "text": (
                    f"{text}\n\nusage={json.dumps(self._last_response_usage, sort_keys=True)}"
...
def _normalize_instruction_qa_imposed_questions(raw: Any) -> list[str]:
    ...
    seen: set[str] = set()
    for value in values:
        question = _clean_instruction_qa_question_text(value)
        if not question:
            continue
